 ...
 */
void LDWS_AllocModel(void) {
	Donnees = (Fichier *) my_malloc(sizeof(Fichier));
	memset(Donnees, 0, sizeof(Fichier));
	M_3D_init = (Modele_3D *) my_malloc(sizeof(Modele_3D));
	memset(M_3D_init, 0, sizeof(Modele_3D));
	M_3D_est = (Modele_3D *) my_malloc(sizeof(Modele_3D));
	memset(M_3D_est, 0, sizeof(Modele_3D));
	M_Filter = (Modele_Filter *) my_malloc(sizeof(Modele_Filter));
	memset(M_Filter, 0, sizeof(Modele_Filter));
	L_output = (LDWS_Output *) my_malloc(sizeof(LDWS_Output));
 ...
 */
void LDWS_FreeResult(LDWS_Output **pLDWSOutput) {
	my_free((*pLDWSOutput)->pPoint);
	(*pLDWSOutput)->pPoint = NULL;

	my_free((*pLDWSOutput)->p3DPoint);
	(*pLDWSOutput)->p3DPoint = NULL;

	my_free((*pLDWSOutput)->pCaPoint);
	(*pLDWSOutput)->pCaPoint = NULL;

	my_free((*pLDWSOutput)->p3DCaPoint);
	(*pLDWSOutput)->p3DCaPoint = NULL;

	my_free((*pLDWSOutput)->Param);
...
void LDWS_Getinit(LDWS_InitGuid **pLDWSInit) {
	...
	if ((*pLDWSInit) == NULL) {
		*pLDWSInit = (LDWS_InitGuid *) my_malloc(sizeof(LDWS_InitGuid));
		...
		(*pLDWSInit)->pBoundPoint = (LDWS_Point *) my_malloc(
				7 * sizeof(LDWS_Point));
		memset((*pLDWSInit)->pBoundPoint, 0, 7 * sizeof(LDWS_Point));

		(*pLDWSInit)->Param = (double *)my_malloc(
			LDWS_NB_PARAM * sizeof(double));
 ...
 */
void LDWS_Freeinit(LDWS_InitGuid **pLDWSInit) {
	my_free((*pLDWSInit)->pCaPoint);
	(*pLDWSInit)->pCaPoint = NULL;

	my_free((*pLDWSInit)->pBoundPoint);
	(*pLDWSInit)->pBoundPoint = NULL;

	if ((*pLDWSInit)->Param != NULL)
	{
		my_free((*pLDWSInit)->Param);
...
void LDWS_CalVanishPointSet(void) {
	...
	if (intflag == 0) {
		pLDWSVPoint.hist = (int *) my_malloc(
